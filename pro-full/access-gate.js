...
function paint(){
  document.title='切削加工条件計算 PRO';
  const tag=document.querySelector('.head .tag');if(tag){tag.textContent='PRO';tag.style.background='#d4af37';tag.style.color='#202226';tag.style.padding='4px 10px';tag.style.borderRadius='99px';tag.style.fontSize='12px';tag.style.fontStyle='normal';tag.style.letterSpacing='.5px';tag.style.transform='none'}
  const h=document.querySelector('.head h1');if(h)h.textContent='切削加工条件計算 PRO';
  const remain=Math.max(0,end-now),days=Math.ceil(remain/(24*60*60*1000));
  if(!document.getElementById('proTrialStatus')){const box=document.createElement('div');box.id='proTrialStatus';box.style.cssText='max-width:620px;margin:10px auto 0;padding:10px 14px;box-sizing:border-box;background:#282b30;border:1px solid #8a6f22;border-radius:10px;color:#ffe28a;font-weight:800;font-size:13px';box.textContent=`PRO体験版｜有効期限：あと${days}日`;const main=document.querySelector('main.w');if(main)main.insertAdjacentElement('afterbegin',box)}
  document.querySelectorAll('.card.note').forEach(x=>{if(/EXTRA版/.test(x.textContent||''))x.innerHTML='<b>PRO 7日間体験版</b><br>初回利用から7日間ご利用いただけます。'})
}
