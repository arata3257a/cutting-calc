...
function filteredRows(){const rows=load();const mat=byId('historyMatFilter')?.value||'';const q=(byId('historySearch')?.value||'').trim().toLowerCase();return rows.filter(r=>{if(mat&&r.mat!==mat)return false;if(!q)return true;const hay=[r.memo,r.matLabel,r.opLabel,r.cool,r.d,r.z,r.vc,r.fz,r.ap,r.ae,r.n,r.f].join(' ').toLowerCase();return hay.includes(q)})}
function syncMaterialFilter(){const sel=byId('historyMatFilter');if(!sel)return;const keep=sel.value;const mats=[...new Map(load().filter(r=>r.mat).map(r=>[r.mat,r.matLabel||r.mat])).entries()].sort((a,b)=>String(a[1]).localeCompare(String(b[1]),'ja'));sel.innerHTML='<option value="">すべての材質</option>'+mats.map(([v,l])=>`<option value="${esc(v)}">${esc(l)}</option>`).join('');if(mats.some(([v])=>v===keep))sel.value=keep}
function render(){const box=byId('historyList');if(!box)return;const all=load();syncMaterialFilter();const rows=filteredRows();byId('historyCount').textContent=rows.length===all.length?`${all.length}件保存`:`${rows.length} / ${all.length}件表示`;if(!all.length){box.innerHTML='<div class="hx-empty">まだ保存履歴はありません。</div>';return}if(!rows.length){box.innerHTML='<div class="hx-empty">条件に一致する履歴はありません。</div>';return}box.innerHTML=rows.map(r=>`<div class="hx-row"><div class="hx-top"><b>${esc(r.memo||r.matLabel||'加工条件')}</b><span>${esc(nowLabel(r.createdAt))}</span></div><div class="hx-sub">${esc(r.matLabel)} / ${esc(r.opLabel)} / φ${esc(r.d)} / ${esc(r.z)}枚刃</div><div class="hx-values">N ${Number(r.n||0).toLocaleString()} rpm　F ${Number(r.f||0).toLocaleString()} mm/min　ap ${esc(r.ap)}　ae ${esc(r.ae)}</div><div class="hx-actions"><button data-restore="${esc(r.id)}">再呼び出し</button><button class="danger" data-delete="${esc(r.id)}">削除</button></div></div>`).join('');box.querySelectorAll('[data-restore]').forEach(b=>b.onclick=()=>restore(b.dataset.restore));box.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>removeOne(b.dataset.delete))}
function build(){if(byId('historyCard'))return;const style=document.createElement('style');style.textContent='.hx-tools{display:grid;grid-template-columns:1fr 1fr;gap:8px}.hx-filter{display:grid;grid-template-columns:1fr 1.4fr;gap:8px;margin-top:10px}.hx-filter select,.hx-filter input{padding:10px;font-size:14px}.hx-tools button,.hx-actions button{border:0;border-radius:9px;padding:11px;font-weight:800}.hx-primary{background:#d4af37;color:#2b2d30}.hx-secondary{background:#5b6067;color:#fff}.hx-row{background:#393d42;border:1px solid #5c6168;border-radius:10px;padding:10px;margin-top:9px}.hx-top{display:flex;justify-content:space-between;gap:8px;align-items:start}.hx-top span,.hx-sub,.hx-values,.hx-empty{font-size:11px;color:#c0c4c8}.hx-sub{margin-top:4px}.hx-values{margin-top:5px;color:#f1f3f5}.hx-actions{display:grid;grid-template-columns:1fr auto;gap:8px;margin-top:8px}.hx-actions button{background:#5b6067;color:white;padding:9px}.hx-actions .danger{background:#70434a}.hx-count{font-size:11px;color:#c0c4c8;font-weight:400;margin-left:6px}@media(max-width:390px){.hx-tools,.hx-filter{grid-template-columns:1fr 1fr}}';document.head.appendChild(style);
 const card=document.createElement('div');card.id='historyCard';card.className='card';card.innerHTML='<div class="sec">④ 条件を保存・履歴 <span id="historyCount" class="hx-count"></span></div><label>メモ（任意）</label><input id="historyMemo" type="text" maxlength="60" placeholder="例：SUS304 荒加工 良好条件"><button id="historySave" class="btn" type="button">この条件を保存</button><div class="hx-filter"><select id="historyMatFilter"><option value="">すべての材質</option></select><input id="historySearch" type="search" placeholder="メモ・加工方法・数値で検索"></div><div class="hx-tools" style="margin-top:9px"><button id="historyCsv" class="hx-secondary" type="button">CSV出力</button><button id="historyBackup" class="hx-secondary" type="button">バックアップ</button><button id="historyImportBtn" class="hx-secondary" type="button">バックアップ読込</button><button id="historyClear" class="hx-secondary" type="button">履歴を全削除</button></div><input id="historyImport" type="file" accept="application/json,.json" class="hide"><div id="historyMsg" class="note" style="margin-top:9px"></div><div id="historyList"></div>';
 ...
 byId('historySave').onclick=saveCurrent;byId('historyCsv').onclick=exportCsv;byId('historyBackup').onclick=exportJson;byId('historyImportBtn').onclick=()=>byId('historyImport').click();byId('historyImport').onchange=e=>{const f=e.target.files?.[0];if(f)importJson(f);e.target.value=''};byId('historyMatFilter').onchange=render;byId('historySearch').oninput=render;byId('historyClear').onclick=()=>{if(confirm('保存した履歴をすべて削除しますか？')){localStorage.removeItem(KEY);render();setMsg('履歴をすべて削除しました。')}};render()
}
