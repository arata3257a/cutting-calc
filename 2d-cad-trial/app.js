...
const TRIAL_EXPIRES_AT = Date.parse("2026-09-27T23:59:59+09:00");
const TRIAL_SESSION_KEY = "easy-2d-cad-trial-auth-202609";
const TRIAL_DOWNLOAD_KEY = "easy-2d-cad-trial-download-202609";

function trialExpired(){
  return Date.now()>TRIAL_EXPIRES_AT;
}

function refreshTrialDownloadState(){
  const used=localStorage.getItem(TRIAL_DOWNLOAD_KEY)==="1";
  document.querySelectorAll("#outputMenu button").forEach(btn=>btn.disabled=used);
  const summary=document.querySelector("#outputMenu summary");
  if(summary) summary.textContent=used?"📤 出力（使用済み）":"📤 出力";
  return used;
}

function unlockTrial(){
  document.body.classList.remove("trial-locked");
  qs("trialGate")?.classList.add("hidden");
  refreshTrialDownloadState();
}

function initTrialGate(){
  const gate=qs("trialGate");
  const input=qs("trialPassword");
  const button=qs("trialUnlockBtn");
  const message=qs("trialGateMessage");
  if(!gate||!input||!button) return;

  if(trialExpired()){
    if(qs("trialGateInfo")) qs("trialGateInfo").textContent="このお試し版は2026年9月27日で終了しました。";
    input.disabled=true;
    button.disabled=true;
    button.textContent="期限終了";
    if(message) message.textContent="試用期間が終了しています。";
    return;
  }

  if(sessionStorage.getItem(TRIAL_SESSION_KEY)==="1"){
    unlockTrial();
    return;
  }
...
function openExportSavePanel(format){
  if(trialExpired()){
    alert("このお試し版の有効期限は終了しました。");
    return;
  }
  if(localStorage.getItem(TRIAL_DOWNLOAD_KEY)==="1"){
    alert("このお試し版では出力保存は1回までです。");
    refreshTrialDownloadState();
    return;
  }
  exportFormat=format;
...
async function confirmExportSave(){
  ...
  try{
    ...
    localStorage.setItem(TRIAL_DOWNLOAD_KEY,"1");
    refreshTrialDownloadState();
    qs("fileSavePanel")?.classList.add("hidden");
    closePdfPreview(false);
...
if ("serviceWorker" in navigator) {
  ...
  window.addEventListener("load", () => {
    navigator.serviceWorker
      .register("./sw.js?v=134t1",{updateViaCache:"none"})
      .then(reg=>reg.update())
      .catch(() => {});
  });
}
